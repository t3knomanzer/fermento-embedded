...
class BaseScreen(Screen):
    def __init__(self):
        ...
        Button(
            wri_icons, 2, 2, text="E", callback=self.new, args=(wri,), **buttons
        )  # New
        Button(
            wri_icons, row := 30, col := 2, text="D", callback=self.replay, **buttons
        )  # Replay
        Button(
            wri_icons, row, col := col + 25, text="F", callback=self.play_cb, **buttons
        )  # Play
        Button(
            wri_icons, row, col := col + 25, text="B", callback=self.pause, **buttons
        )  # Pause
        Button(
            wri_icons, row, col := col + 25, text="A", callback=self.stop, **buttons
        )  # Stop
        Button(
            wri_icons, row, col + 25, text="C", callback=self.skip, **buttons
        )  # Skip
        row = 60
        col = 2
                ...
                async with Screen.rfsh_lock:  # Lock out refresh
                    for n in range(_RFSH_GATE):  # for _RFSH_GATE buffers full
                        if not (num_read := wav.readinto(wav_samples_mv)):  # Song end
                            return
                        I2S.shift(
                            buf=wav_samples_mv[:num_read], bits=16, shift=self.volume
                        )
