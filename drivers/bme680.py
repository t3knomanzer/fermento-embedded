...
class Adafruit_BME680:
    ...
    def _perform_reading(self) -> None:
        ...
        # Init last reading (seconds)
        if getattr(self, "_last_reading", None) is None:
            self._last_reading = now_s - self._min_refresh_time

        # Enforce min refresh (seconds)
        if now_s - self._last_reading < self._min_refresh_time:
            return
        ...
        # store in seconds (monotonic-compatible)
